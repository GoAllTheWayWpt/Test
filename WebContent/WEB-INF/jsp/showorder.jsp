        ...
        <div align="center" id="cart">
            <table class="maintable" border="1" style="text-align: center">
                <tbody>
                    ...
                    <c:if test="${!empty orders }">>
                    <c:forEach items="${orders }" var="pro" varStatus="count">
                    <tr>
                        <td width="26%">${pro.id }</td>
                        <td width="50%">
                            <table style="border-collapse: collapse;border:#f8e49b;width:100%;text-align:center;" border="1">
                                <tbody>
                                    ...
                                    <c:forEach items="${pro.orderItem }" var="item" varStatus="count">
                                    <tr>
                                        <td>${item.product.name }</td>
                                        <td>${item.product.price }</td>
                                        <td>${item.buynum }</td>
                                         <c:if test="${count.count==1 }">
		                                       	 <td rowspan="10">${pro.money }</td>
                        ...
                        <td>
                            <a href="/bb/pay.jsp?id=a5f10151-6841-4e13-a4c3-570af3d8968f&amp;money=279.0">${pro.paysate }</a>
                        </td>
